 ...
 * Renders a realistic smartphone frame with dynamic live profile preview.
 */
export default function PhonePreview({
  ...
  return (
    <div className="sticky top-6 space-y-4">
      ...
      <div className="relative mx-auto w-[280px] sm:w-[300px] h-[580px] sm:h-[620px] bg-slate-900 rounded-[44px] p-3 shadow-2xl ring-1 ring-slate-800/80 ring-offset-4 ring-offset-slate-100 flex flex-col justify-between select-none">
        ...
        >
          <div className="w-full flex flex-col items-center">
            {/* Avatar */}
            <div className="w-20 h-20 rounded-full ring-2 ring-white/30 shadow-lg overflow-hidden shrink-0 mb-3">
            ...
            {/* Bio */}
            {bio && (
              <p className={`text-[11px] ${currentTheme.subtitleColor} text-center mt-2 px-2 line-clamp-3 leading-snug`}>
                {bio}
              </p>
            )}
            ...
            {buttonKeys.length > 0 && (
              <div className="flex flex-wrap gap-2 justify-center my-3">
                {buttonKeys.map((key) => (
                  <div
                    key={key}
                    className={`w-7 h-7 rounded-full flex items-center justify-center ${currentTheme.buttonStyle} shadow-2xs backdrop-blur-xs text-[11px]`}
                  >
            ...
            <div className="w-full space-y-2 mt-2">
              {visibleLinks.length === 0 ? (
                <div className="text-center py-6">
                  <p className={`text-[11px] ${currentTheme.mutedTextColor} opacity-60 italic`}>
                    No active links yet
                  </p>
                </div>
              ) : (
