...
import { cancelRazorpayAtPeriodEndAction } from '@/action/BillingAction';
...
export default function BillingClient({ presentation }) {
  ...
  const [isCancellationScheduled, setIsCancellationScheduled] = useState(presentation?.cancelAtPeriodEnd ?? false);

  const isPro = presentation?.isPro ?? false;
  const isManualPro = presentation?.isManualPro ?? false;
  const canCancelSubscription = Boolean(presentation?.canCancelSubscription && !isCancellationScheduled);
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              >
                {isCancellationScheduled ? 'Cancellation scheduled' : presentation.statusBadge}
              </span>
            </div>
              ...
              {isPro
                ? (isManualPro
                    ...
                    : (isCancellationScheduled
                        ? 'Cancellation is scheduled. Your Pro access remains active through your current paid billing period.'
          ...
          {presentation.periodEndLabel && (
            <div className="bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-left sm:text-right shrink-0">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 block">
                {isCancellationScheduled || presentation.cancelAtPeriodEnd ? 'Access Valid Until' : 'Renewal / Period End'}
              </span>
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              {isPro ? (
                <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-xs font-bold px-3 py-1 rounded-full">
                  {isManualPro ? 'Manual Grant' : (isCancellationScheduled ? 'Cancellation Scheduled' : 'Active')}
                </span>
              ) : (
                ...
                <div className="space-y-3">
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-3.5 space-y-1">
                    ...
                    <p className="text-[11px] text-amber-200/90 leading-relaxed">
                      {presentation.periodEndLabel
                        ? `Your Pro access remains active until ${presentation.periodEndLabel}. You will not be charged for another billing cycle.`
                        : 'Your Pro access remains active until the end of your billing cycle. You will not be charged for another cycle.'}
                    </p>
                  </div>
                  <button
                    type="button"
                    disabled
                    className="w-full py-3.5 px-4 rounded-2xl text-xs font-bold bg-amber-500/20 text-amber-300 border border-amber-500/40 cursor-default"
                  >
                    Pro Active (Cancellation Scheduled)
                  </button>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  {canCancelSubscription && (
                    <button
                      ...
                      className="w-full py-2.5 px-4 rounded-2xl text-xs font-semibold text-rose-300/80 hover:text-rose-200 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 transition-all cursor-pointer"
                    >
                      Cancel Subscription
                    </button>
                  )}
      ...
      {isCancelModalOpen && (
        ...
        >
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-slate-100 space-y-6 relative text-slate-900">
            <button
              type="button"
            ...
            <div className="space-y-2">
              ...
              <h3 id="cancel-modal-title" className="text-xl font-bold text-slate-900">
                Cancel Pro subscription?
              </h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                Your Pro features will remain available through the current paid billing period. The subscription will not renew after that period.
              </p>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3.5 space-y-1">
              ...
              <p className="text-[11px] text-amber-700 leading-relaxed">
                No real money was charged. This will schedule test cancellation at cycle end with Razorpay.
              </p>
            </div>

            {cancelErrorMessage && (
              <div className="bg-rose-50 border border-rose-200 text-rose-700 text-xs rounded-2xl p-3">
                {cancelErrorMessage}
              </div>
            )}
            ...
            <div className="space-y-2 pt-2">
              <button
                ...
                onClick={async () => {
                  ...
                  setCancelErrorMessage(null);
                  try {
                    const res = await cancelRazorpayAtPeriodEndAction();
                    if (res?.success) {
                      setIsCancellationScheduled(true);
                      setIsCancelModalOpen(false);
                    } else {
                      setCancelErrorMessage(res?.message || 'Failed to schedule cancellation.');
                    }
                  } catch (err) {
                    setCancelErrorMessage(err?.message || 'An unexpected error occurred.');
                  } finally {
                    setIsCancelling(false);
                  }
                }}
                className="w-full py-3.5 px-4 rounded-2xl text-xs font-bold bg-rose-600 hover:bg-rose-700 text-white transition-all shadow-xs cursor-pointer disabled:opacity-50"
              >
                {isCancelling ? 'Scheduling cancellation...' : 'Cancel at period end'}
              </button>
              <button
                type="button"
                disabled={isCancelling}
                onClick={() => setIsCancelModalOpen(false)}
                className="w-full py-3 px-4 rounded-2xl text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors cursor-pointer disabled:opacity-50"
              >
                Keep Pro
              </button>
            </div>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-slate-100 space-y-6 relative">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                Prince Links Pro offers advanced customization and monetization features for creators.
              </p>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 space-y-2 text-xs text-slate-600">
              ...
              <p className="leading-relaxed">
                All existing baseline features (tip jars, rich embeds, typography, themes, analytics) remain permanently available for free.
              </p>
            </div>
