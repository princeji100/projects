...
export default function YouTubeEmbed({
  ...
  if (!link || !media || media.provider !== 'youtube' || !media.id) {
    return null;
  }
  ...
  // Safe HTTPS privacy-enhanced embed URL
  const embedSrc = `https://www.youtube-nocookie.com/embed/${encodeURIComponent(media.id)}?playsinline=1`;
  const titleText = link.title || 'YouTube Video';
  const subtitleText = link.subtitle || (media.kind === 'shorts' ? 'YouTube Shorts' : 'YouTube Video');
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 w-full">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          {/* Custom Link Icon or YouTube Branded Icon */}
          <div
            className={`${currentTheme?.iconBg || 'bg-red-600'} w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center overflow-hidden shrink-0 ring-1 ring-white/20 shadow-inner`}
          >
            {link.icon ? (
              <LinkIcon src={link.icon} title={titleText} />
            ) : (
              <FontAwesomeIcon icon={faYoutube} className="text-xl sm:text-2xl text-white" />
            )}
          </div>
        ...
        {/* Right Side: Play / Collapse Action & Escape Hatch */}
        <div className="flex items-center gap-2 shrink-0">
          {!isPlayerMounted ? (
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-red-600 hover:bg-red-700 active:scale-95 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-sm cursor-pointer min-h-[40px] focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none"
            >
          ...
          <a
            ...
            aria-label={`Open ${titleText} on YouTube`}
      ...
      {/* Lazy Mounted Responsive 16:9 Iframe Player */}
      {isPlayerMounted && (
