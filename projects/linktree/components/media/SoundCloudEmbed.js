...
import { buildSoundCloudEmbedUrl } from '@/lib/mediaEmbeds';

/**
 * Returns canonical static player height depending on SoundCloud resource kind.
 *
 * @param {string} kind
 * @returns {number}
 */
function getSoundCloudEmbedHeight(kind) {
  switch (kind) {
    case 'track':
      return 166;
    case 'set':
...
export default function SoundCloudEmbed({
  ...
  if (!link || !media || media.provider !== 'soundcloud' || !media.canonicalUrl) {
    return null;
  }

  const embedSrc = buildSoundCloudEmbedUrl(media);
  if (!embedSrc) {
    return null;
  }
  ...
  const titleText = link.title || 'SoundCloud Audio';
  const kindLabel = media.kind === 'set' ? 'Playlist' : 'Track';
  const subtitleText = link.subtitle || `SoundCloud ${kindLabel}`;
  const embedHeight = getSoundCloudEmbedHeight(media.kind);
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 w-full">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          {/* Custom Link Icon or SoundCloud Branded Icon */}
          <div
            className={`${currentTheme?.iconBg || 'bg-[#ff5500]'} w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center overflow-hidden shrink-0 ring-1 ring-white/20 shadow-inner`}
          >
            {link.icon ? (
              <LinkIcon src={link.icon} title={titleText} />
            ) : (
              <FontAwesomeIcon icon={faSoundcloud} className="text-xl sm:text-2xl text-white" />
            )}
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {!isPlayerMounted ? (
            <button
              ...
              aria-label={`Open SoundCloud player for ${titleText}`}
              className="px-3.5 py-2 rounded-xl bg-[#ff5500] hover:bg-[#e04b00] active:scale-95 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-sm cursor-pointer min-h-[40px] focus-visible:ring-2 focus-visible:ring-[#ff5500] focus-visible:outline-none"
            >
          ...
          <a
            ...
            aria-label={`Open ${titleText} on SoundCloud`}
      ...
      {/* Lazy Mounted Responsive SoundCloud Iframe Player */}
      {isPlayerMounted && (
