...
/**
 * Returns canonical static player height depending on Spotify resource kind.
 *
 * @param {string} kind
 * @returns {number}
 */
function getSpotifyEmbedHeight(kind) {
  switch (kind) {
    case 'track':
    case 'episode':
      return 152;
...
export default function SpotifyEmbed({
  ...
  if (!link || !media || media.provider !== 'spotify' || !media.id || !media.kind) {
    return null;
  }
  ...
  // Safe HTTPS Spotify embed URL
  const embedSrc = `https://open.spotify.com/embed/${encodeURIComponent(media.kind)}/${encodeURIComponent(media.id)}`;
  const titleText = link.title || 'Spotify Music';
  const kindLabel = media.kind.charAt(0).toUpperCase() + media.kind.slice(1);
  const subtitleText = link.subtitle || `Spotify ${kindLabel}`;
  const embedHeight = getSpotifyEmbedHeight(media.kind);
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 w-full">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          {/* Custom Link Icon or Spotify Branded Icon */}
          <div
            className={`${currentTheme?.iconBg || 'bg-[#1DB954]'} w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center overflow-hidden shrink-0 ring-1 ring-white/20 shadow-inner`}
          >
            {link.icon ? (
              <LinkIcon src={link.icon} title={titleText} />
            ) : (
              <FontAwesomeIcon icon={faSpotify} className="text-xl sm:text-2xl text-white" />
            )}
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {!isPlayerMounted ? (
            <button
              ...
              aria-label={`Open Spotify player for ${titleText}`}
              className="px-3.5 py-2 rounded-xl bg-[#1DB954] hover:bg-[#1aa34a] active:scale-95 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-sm cursor-pointer min-h-[40px] focus-visible:ring-2 focus-visible:ring-[#1DB954] focus-visible:outline-none"
            >
          ...
          <a
            ...
            aria-label={`Open ${titleText} on Spotify`}
      ...
      {/* Lazy Mounted Responsive Spotify Iframe Player */}
      {isPlayerMounted && (
